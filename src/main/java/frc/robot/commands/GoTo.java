...
public class GoTo {
        ...
        private final static int[] allTagIDs = {10, 9, 11, 7, 8, 6, 1, 2, 3, 21, 22, 20, 18, 17, 19, 13, 12, 16,};
        ...
        public static int bestTag(Pose2d robotPose) {
            return Collections.min(
                    ...
                    Comparator.comparing(
                            (Integer other) -> robotPose.getTranslation().getDistance(getTagPose(other).getTranslation()))
                    .thenComparing(
                            (Integer other) ->
                            Math.abs(robotPose.getRotation().minus(getTagPose(other).getRotation()).getRadians())));
        }
