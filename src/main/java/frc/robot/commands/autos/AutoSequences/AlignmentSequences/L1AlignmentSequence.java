...
public class L1AlignmentSequence extends SequentialCommandGroup {
    ...
    public L1AlignmentSequence(CoralArm coralArm, AlgaeArm algaeArm,
            Elevator elevator, SwerveSubsystem swerveSubsystem,
            NTDouble strafeOffset, NTDouble distanceOffset,
            NTDouble rotationOffset) {
        var config = new ConfigSystem(Constants.SetpointConstants.Options.l1,
        ...
        var scoreAlign = new AutoAlignReef(swerveSubsystem, strafeOffset,
                distanceOffset, rotationOffset, NTD.of(0.02), NTD.of(0.02));
        var scoreCoral = new AutoCoralScore(coralArm);

        addCommands(new ParallelCommandGroup(configureAlign, config),
                scoreAlign, scoreCoral.withTimeout(0.5), secondConfigureAlign,
                stow);
    }
}
